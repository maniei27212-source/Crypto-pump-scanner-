...
session.headers.update({
    "User-Agent": "Crypto-Pump-Scanner/4.0"
})
...
def pre_pump_filter(data):
    ...
    # Don't chase already vertical moves
    if change_5m >= 60:
...
def score_candidate(data):
    ...
    data_volume_ratio = volume_ratio
...
def load_binance_symbols():
    ...
    if BINANCE_SYMBOL_CACHE is not None:
...
def evaluate_buy_now(data):
    ...
    if volume_ratio < 1.20:
        ...
        failures.append(
            f"Volume acceleration {volume_ratio:.2f}x < 1.20x"
        )
    ...
    if change_5m > 25:
        ...
        failures.append(
            f"5M already extended +{change_5m:.1f}%"
        )
    ...
    if change_1h > 50:
        ...
        failures.append(
            f"1H already extended +{change_1h:.1f}%"
        )
...
def evaluate_early_radar(data):
    ...
    if volume_ratio >= 1.3:
        ...
        reasons.append(
            f"Volume acceleration {volume_ratio:.2f}x"
        )
...
def scan():
    ...
    print(
        "Starting Crypto Pump Scanner v4..."
    )
